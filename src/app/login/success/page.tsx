...
export default function Page({ searchParams }: { searchParams: { chatId?: string } }) {
    if(searchParams.chatId) return <LoginSuccessPage chatId={searchParams.chatId} />

    return (
        <div className="h-screen flex justify-center items-center bg-gray-100">
            <Card className="p-10 bg-red-400 text-white">
                <CardTitle>Ocurrió un error. Lo sentimos</CardTitle>
                <CardDescription className="text-white">Recomendamos cerrar la aplicación y volver a acceder desde tu asistente.</CardDescription>
            </Card>
        </div>
    )
}
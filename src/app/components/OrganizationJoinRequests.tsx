                ...
                <Card key={req.request_id} className='p-0'>
                    <CardContent className="flex items-center justify-between gap-4 p-0 py-3 px-2">
                        <div className='flex justify-center'>
                            <p className="font-normal text-sm break-all indent-0.5">{req.email}</p>
                        </div>
                        <div className="flex gap-1">
                            <Button size="icon" variant="outline" onClick={() => handleAction(req.request_id, 'approved')} className='w-6 h-6'>
                                <Check className="w-4 h-4 text-green-600" />
                            </Button>
                            <Button size="icon" variant="outline" onClick={() => handleAction(req.request_id, 'rejected')} className='w-6 h-6'>

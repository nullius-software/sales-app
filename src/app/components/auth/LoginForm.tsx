...
export default function LoginForm() {
  ...
  const handleSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      try {
        await axios.post('/api/telegram/saveChat', {
      ...
      toast.error(
        'Un error inesperado ocurrió. Por favor revisa tu conexión a internet.'
      );
    } finally {
    ...
    <Form {...form}>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input {...field} placeholder="Entra tu email" type="email" />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
      ...
      <p className="text-center text-gray-500 mt-8 p-0">
        ...
        <Link
          ...
          href={`/register?${chatId ? `chatId=${chatId}` : ''}`}
        >
          Hace click acá
        </Link>
      </p>

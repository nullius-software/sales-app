...
export default function MembersList({ currentUser }: { currentUser: User }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (userIsCreator)
        req.push(
          axios.get(`/api/organizations/${currentOrganization.id}/requests`)
        );
    ...
    <Card className="w-full h-full">
      ...
      <CardContent>
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          Esta acción transferirá la propiedad de la
                          organización a {member.email}. Luego de eso ya no
                          podrás realizar acciones administrativas como eliminar
                          miembros.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
                        ...
                        <AlertDialogAction
                          onClick={async () => {
                            ...
                            try {
                              await axios.patch(
                                `/api/organizations/${currentOrganization.id}/transfer`,
                                {
                                  newOwnerId: transferTarget.id,
                                }
                              );
                              toast.success(
                                `La organización fue transferida a ${transferTarget.email}`
                              );
                              setCurrentOrganization({
                                ...currentOrganization,
                              ...
                              toast.error(
                                'Error al transferir la organización'
                              );
                            } finally {
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          Esta acción eliminará permanentemente al miembro de la
                          organización. Esto no se puede deshacer.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
        ...
        {members.length === 0 && requests.length === 0 && (
          <p>
            No hay miembros ni peticiones de ingreso. <br />
            {userIsCreator &&
              'Cuando tus empleados soliciten entrar a tu organización lo verás aquí.'}
          </p>
        )}
      </CardContent>
      <PaginationControls

...
export async function POST(request: Request) {
    ...
    const { data: { access_token } } = await axios.post(process.env.KEYCLOAK_URL + '/realms/nullius-realm/protocol/openid-connect/token',
        {
    ...
    try {
        await axios.post(process.env.KEYCLOAK_URL + "/admin/realms/nullius-realm/users", {
            email,
            enabled: true,
            credentials: [
                {

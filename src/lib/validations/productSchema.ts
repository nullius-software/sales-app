...
export const getProductSchema = (stockDecimal: boolean) => z.object({
    ...
    stock: z.number({
        ...
        .refine((val) => stockDecimal ? /^\d+(\.\d{1,2})?$/.test(val.toString()) : Number.isInteger(val), stockDecimal ? 'El stock debe tener hasta 2 decimales' : 'El stock debe ser entero'),
